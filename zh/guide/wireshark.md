...
When you can access the remote machine with SSH you can stream the tcpdump to your local machine instead of logging it to a file. <em x-id="3">Wireshark</em> can open this stream and show the decoded MAVLink messages using the tools and filters from above.
...
3. Start the data stream on the remote machine and pipe it into the named pipe on your local machine.
   ...
   - `-i lo` Define which interface to listen on. <code>-i lo</code> Define which interface to listen on. This will listen to the loopback interface, you can change this to the Ethernet, USB or modem interface.
