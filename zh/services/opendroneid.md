...
Particularly the broadcast method used with Bluetooth Legacy Advertising signals impose a strict size limitation for the amount of data that can be transmitted in each radio burst. 因此，将相关数据划分为不同的类别，每个类别通过自己的消息进行传输。

The ASTM Remote ID standard defines 6 such messages and an additional 7th message type for packing multiple messages together into a message pack (used when transmitting on WiFi NaN or Bluetooth Long Range with Extended Advertising). 为了支持无人机ID发送/接收之间的数据传输，支持无人机ID消息所有字段的MAVLink消息已经可用。
