...
The sequence should be run first with `COMP_METADATA_TYPE_VERSION` to get the set of metadata types supported by the component (see `supportedCompMetadataTypes` key in the version metadata). It can then be run for the other types once they are known.
...
1. GCS downloads the file specified in the `metadata_uri` (also the translation files, if any). For `COMP_METADATA_TYPE_VERSION` the file must be hosted on the vehicle and downloaded using [MAVLink FTP](../services/ftp.md).
