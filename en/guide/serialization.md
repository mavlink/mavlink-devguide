...
5 | `uint8_t sysid`  | System ID (sender)     | 1 - 255 | ID of *system* (vehicle) sending the message. Used to differentiate systems on network. Must be non-zero as 0 is a broadcast address.
6 | `uint8_t compid` | Component ID (sender)   | 1 - 255 | ID of *component* sending the message. Used to differentiate *components* in a *system* (e.g. autopilot and a camera). Use values in [MAV_COMPONENT](../messages/common.html#MAV_COMPONENT) except for the broadcast address `MAV_COMP_ID_ALL` (as this is an invalid/meaningless *source* address).
