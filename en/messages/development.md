...
### DISTANCE_SENSOR_INFO (516) — [WIP] {#DISTANCE_SENSOR_INFO}

<span class="warning">**WORK IN PROGRESS**: Do not use in stable production environments (it may change).</span>

Static characteristics of a distance sensor (onboard rangefinder).

Measurements are sent in [DISTANCE_SENSOR_V2](#DISTANCE_SENSOR_V2).

Field Name | Type | Units | Values | Description
--- | --- | --- | --- | ---
id | `uint8_t` | | | Sensor ID. Matches the id field of [DISTANCE_SENSOR_V2](#DISTANCE_SENSOR_V2).<br>[Instance field]: Uniquely identifies a device/subcomponent within a single source/target MAVLink component. 
type | `uint8_t` | | [MAV_DISTANCE_SENSOR](#MAV_DISTANCE_SENSOR) | Type of distance sensor. 
min_distance | `float` | m | invalid:NaN | Minimum distance the sensor can measure. 
max_distance | `float` | m | invalid:NaN | Maximum distance the sensor can measure. 
orientation | `uint8_t` | | [MAV_SENSOR_ORIENTATION](#MAV_SENSOR_ORIENTATION) | Direction the sensor faces. 
horizontal_fov | `float` | rad | invalid:NaN min:0 | Horizontal Field of View (angle) where the distance measurement is valid. NaN if unknown. 
vertical_fov | `float` | rad | invalid:NaN min:0 | Vertical Field of View (angle) where the distance measurement is valid. NaN if unknown. 
<span class='ext'>quaternion</span> <a href='#mav2_extension_field'>++</a> | `float[4]` | | invalid:[0] | Quaternion of the sensor orientation in vehicle body frame (w, x, y, z order, zero-rotation is 1, 0, 0, 0). Zero-rotation is along the vehicle body x-axis. This field is required if orientation is set to [MAV_SENSOR_ROTATION_CUSTOM](#MAV_SENSOR_ROTATION_CUSTOM). Set it to 0 if invalid. 


### DISTANCE_SENSOR_V2 (517) — [WIP] {#DISTANCE_SENSOR_V2}

<span class="warning">**WORK IN PROGRESS**: Do not use in stable production environments (it may change).</span>

Distance sensor measurement, for an onboard rangefinder described by [DISTANCE_SENSOR_INFO](#DISTANCE_SENSOR_INFO). Streamed at the sensor update rate.

Field Name | Type | Units | Description
--- | --- | --- | ---
time_boot_ms | `uint32_t` | ms | Timestamp (time since system boot) 
id | `uint8_t` | | Sensor ID. Matches the id field of [DISTANCE_SENSOR_INFO](#DISTANCE_SENSOR_INFO).<br>[Instance field]: Uniquely identifies a device/subcomponent within a single source/target MAVLink component. 
distance | `float` | m | Current distance reading. NaN if not provided. 
signal_quality | `int8_t` | % | Signal quality (sensor-type specific). Represents the relation of the signal strength with the target reflectivity, distance, size or aspect, but normalised as a percentage. Values: [0-100], 0 = unusable signal, 100 = perfect signal. INT8_MAX = unknown/not provided. 
